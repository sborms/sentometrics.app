...
ui <- fluidPage(theme = shinytheme("cerulean"), shinyjs::useShinyjs(),
    ...
    tags$style(type = "text/css", ".form-control.shiny-bound-input, .selectize-input {height: 35px;}"),
    tagList(div(id = "loading-content", h2("Loading..."))),
    headerPanel(tags$a(href = "https://github.com/sborms/sentometrics.app",
                       target = "_blank", class = "fa fa-github",
                       style = "font-size:24px", id = "github", " sentometrics.app"),
                windowTitle = "sentometrics.app"),
    sidebarPanel(tags$style(tableHTML::make_css(list(".well",
        "border-width", "5px"))), style = "margin: 5px; padding-top: 10px; padding-bottom: 0px",
        header_ui("header_ui"),
        tags$tr(tags$h4("Word lists")),
        lexicon_ui("lexicon_ui"),
        valence_ui("valence_ui"),
        tags$tr(tags$h4("Aggregation")),
        fluidRow(column(width = 6, howWithin_ui("howWithin_ui")),
                 column(width = 6, howDocs_ui("howDocs_ui"))),
        howTime_ui("howTime_ui")),
    mainPanel(id = "main_panel", fluidRow(style = "margin: 0px",
        tabsetPanel(id = "tabs",
            tabPanel(style = "margin: 0px", title = "Corpus",
                     load_corpus_ui("load_corpus_ui"), render_corpus_ui("corpusTable")),
            tabPanel(style = "margin: 0px", title = "Summary",
                     corpus_summary_ui("corpus_summary_ui")),
            tabPanel(style = "margin: 0px", title = "Sentiment",
                     sentiment_ui("sentiment_ui"), value = "sentimentTab"),
            tabPanel(style = "margin: 0px", title = "Indices",
                     indices_ui("indices_ui"), value = "indicesTab")
            )
        )
      )
    )

myvals <- reactiveValues(selectedLexicons = NULL, selectedValence = NULL,
    useValence = FALSE, lexiconList = list_lexicons,
    valenceList = list_valence_shifters, howWithin = NULL,
    howDocs = NULL, howTime = NULL, valenceMethod = "Unigrams",
    sento_measures = NULL, sentiment = NULL, by = NULL,
    lag = 1)
...
server <- function(input, output, session) {
    ...
    howWithinModule <- callModule(howWithin_server,
        "howWithin_ui")
    observe({
    ...
    corpus <- callModule(create_corpus_server, "",
        corpusFile)
    callModule(render_corpus_server, "corpusTable",
        corpusFile)

    corpusSummaryModule <- callModule(corpus_summary_server,
        "corpus_summary_ui", corpus)

    sentoLexicon <- callModule(build_sento_lexicon,
        "", myvals)
    ...
    observeEvent(input$calcSentimentButton, ignoreInit = FALSE,
        {
            if (is.null(sentoLexicon())) {
                ...
                updateTabsetPanel(session, "tabs",
                  selected = "sentimentTab")
                sentimentModule <- callModule(sentiment_server,
                  "sentiment_ui", myvals, corpus, sentoLexicon,
                  input$calcSentimentButton)
                observe({
